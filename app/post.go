...
func (a *App) DeletePost(postId, deletingUserID string) (*model.Post, *model.AppError) {
	if result := <-a.Srv.Store.Post().GetSingle(postId); result.Err != nil {
		...
		if result := <-a.Srv.Store.Post().Delete(postId, model.GetMillis(), deletingUserID); result.Err != nil {
			return nil, result.Err
		}
